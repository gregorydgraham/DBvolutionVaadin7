 ...
 */
package nz.co.gregs.amhan.database;
...
public class DatabaseComponent extends Div {
	...
	private final Details exceptionDiv = new Details("No Exception", new Div());
	...
	public DatabaseComponent(DBDatabase database) {
		...
		databaseStatusDiv.addClassName("cluster-monitor-database-status");
		exceptionDiv.addThemeName("cluster-monitor-exception");
		...
		if (database != null) {
			...
			if (except != null) {
				exceptionDiv.setSummary(new Paragraph(database.getLastException().getLocalizedMessage().substring(0, 50)));
				exceptionDiv.addContent(new Div(new Label(database.getLastException().getLocalizedMessage())));
				StackTraceElement[] stackTrace = except.getStackTrace();
				for (StackTraceElement stackTraceElement : stackTrace) {
					exceptionDiv.addContent(new Div(new Label(stackTraceElement.toString())));
				}
			} else {
				exceptionDiv.addContent(new Div(new Label("No Exception")));
			}

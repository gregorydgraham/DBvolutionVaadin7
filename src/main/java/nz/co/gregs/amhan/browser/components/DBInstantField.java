...
public class DBInstantField extends AbstractField<DBInstantField, Instant>
		implements HasStyle, HasSize, HasTheme, HasValidation,
		Focusable<DBInstantField> {
	...
	private final static SerializableFunction<String, Instant> PARSER = s -> {
		return s == null || s.isEmpty() ? null : Instant.parse(s);
	};

	private final static SerializableFunction<Instant, String> FORMATTER = d -> {
	...
	/**
	 * Convenience constructor to create a date time picker with pre-selected date
			...
			try {
				super.setValue(
						jsonValue.asString().isEmpty() ? null
								//				: Instant.parse(jsonValue.asString())));
								: TemporalStringParser.toInstant(jsonValue.asString()));
			} catch (ParseException ex) {
				ex.printStackTrace();
				Logger.getLogger(DBInstantField.class.getName()).log(Level.SEVERE, null, ex);
			}
		});
	}
	...
	protected void setPresentationValue(Instant newPresentationValue) {
		final ZoneOffset offset = ZonedDateTime.now().getOffset();
		datePicker.setValue(newPresentationValue != null
				? newPresentationValue.atOffset(offset).toLocalDate()
				: null);
		timePicker.setValue(newPresentationValue != null
				? newPresentationValue.atOffset(offset).toLocalTime()
				: null);
	...
	private void updateValue() {
		...
		final ZoneOffset offset = ZonedDateTime.now().getOffset();
		final Instant newValue = date != null && time != null
	 ...
	 * specifies the intervals for the displayed items in the time picker dropdown
	 * and also the displayed time format.
	 * <p>
	 ...
	 * {@link com.vaadin.flow.component.HasValue.ValueChangeEvent} to be fired if
	 * some parts (eg. seconds) is discarded from the value.</em>
	 *
	 * @param step the step to set, not {@code null} and should divide a day or an
	 * hour evenly
	 */
	...
	/**
	 * Set the Locale for the DateTimePicker. The displayed date and time will be
	 * matched to the format used in that locale.
	 *
	 * @param locale the locale to set to the DateTimePicker, cannot be null
	 */
	...
	/**
	 * Synchronizes the theme attribute to the internal DatePicker and TimePicker
	 * components.
	 */
	 ...
	 *
	 * @param themeName the theme name to remove, not <code>null</code>
	 * @return <code>true</code> if the theme name was removed, <code>false</code>
	...
	/**
	 * Sets the theme names of this component. This method overwrites any previous
	 * set theme names.
	 *
	 * @param themeName a space-separated string of theme names to set, or empty
	 ...
	 *
	 * @param themeName the theme name to set or remove, not <code>null</code>
	 * @param set <code>true</code> to set the theme name, <code>false</code> to
	 * remove it
	 */
	@Override
	...
	/**
	 * Adds one or more theme names to this component. Multiple theme names can be
	 * specified by using multiple parameters.
	 *
	 * @param themeNames the theme name or theme names to be added to the
	 * component
	 */
	@Override
	 ...
	 * specified by using multiple parameters.
	 *
	 * @param themeNames the theme name or theme names to be removed from the
	 * component
	 */
	@Override
	 ...
	 */
	private boolean isInvalid(Instant value) {
	 ...
	 * because it is possible to circumvent the client-side validation constraints
	 ...
	 *
	 * @param min the minimum date and time that is allowed to be set, or
	 ...
	 *
	 * @param max the maximum date and time that is allowed to be set, or
	 * <code>null</code> to remove any minimum constraints
	 */
	 ...
	 *
	 * Note: updating the object content that is gotten from this method will not
	 * update the lang on the component if not set back using
	 ...
	 *
	 * @param requiredIndicatorVisible the value of the requiredIndicatorVisible
	 * to be set
	 */
	@Override
	...
	/* Copied from com.vaadin.flow.component.datetimepicker */
 /* Sorry about that guys                                                       */
	private static final class FieldValidationUtil {
